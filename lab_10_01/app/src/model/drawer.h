...
class Drawer
{
  ...
  public:
    ...
    int imageWidth() { return _image->width(); };
    int imageHeight() { return _image->height(); };

    QColor color() const { return _pen.color(); };
    QColor pixelColor(int x, int y) const { return _image->pixelColor(x, y); }
    ...
    void drawCircle(const QPoint &c, int r) {
        p.drawEllipse(c, r, r);
    }

...
void Canvas::update_scene_rect()
{
    ...
    else
    {
        ...
        sceneRect.setTop(std::max(sceneRect.top(),
                                  points->boundingRect().top()));
        sceneRect.setRight(std::max(sceneRect.right(),
                                    points->boundingRect().right()));
        sceneRect.setBottom(std::min(sceneRect.bottom(),
                                     points->boundingRect().bottom()));
        sceneRect.setLeft(std::min(sceneRect.left(),
                                   points->boundingRect().left()));
    }
...
void Canvas::draw_coord_axes(QPainter &painter)
{
    ...
    if ((sceneRect.bottom() < 0) and (sceneRect.top() > 0))
        painter.drawLine(QLine(QPoint(30, zero_point.y()),
                               QPoint(width(), zero_point.y())));

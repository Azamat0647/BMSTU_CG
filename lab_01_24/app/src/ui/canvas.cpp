...
void Canvas::triangle_not_found_message() {
  QMessageBox msgBox;
  msgBox.setText(
      "с данным множеством точек невозможно построить ни один треугольник!");
...
void Canvas::find_triangle() {
  ...
  printf("\n(%f, %f)\n", max_triangle->orthocenter().x(),
         max_triangle->orthocenter().y());
...
void Canvas::update_scene_rect() {
  ...
  if (show_triangle)
    ...
    sceneRect.setRight(
        std::max(sceneRect.right(), points->boundingRect().right()));
    sceneRect.setBottom(
        std::min(sceneRect.bottom(), points->boundingRect().bottom()));
    sceneRect.setLeft(
        std::min(sceneRect.left(), points->boundingRect().left()));
  }
  ...
  float axes_width = sceneRect.width();
  float axes_height = -sceneRect.height();

  float x_coeff = width() / axes_width;
  float y_coeff = height() / axes_height;

  if (width() / (float)height() > 1) {
    float new_width = width() / y_coeff;

    sceneRect.setLeft(sceneRect.left() - (new_width - axes_width) / 2);
    sceneRect.setRight(sceneRect.right() + (new_width - axes_width) / 2);
  } else if (width() / (float)height() < 1) {
    float new_height = height() / x_coeff;
    sceneRect.setBottom(sceneRect.bottom() - (new_height - axes_height) / 2);
    sceneRect.setTop(sceneRect.top() + (new_height - axes_height) / 2);
  }
}

SetOfPoints *Canvas::getPoints() { return points; }
...
void Canvas::draw_coord_axes(QPainter &painter) {
  ...
  if ((sceneRect.bottom() < 0) and (sceneRect.top() > 0))
    painter.drawLine(
        QLine(QPoint(30, zero_point.y()), QPoint(width(), zero_point.y())));

  if ((sceneRect.left() < 0) and (sceneRect.right() > 0))
    painter.drawLine(QLine(QPoint(zero_point.x(), 0),
                           QPoint(zero_point.x(), height() - 20)));
  ...
  for (float xi = ((int)(sceneRect.left() / step)) * step;
       xi < sceneRect.right(); xi += step) {
    p = point_transform_to_display(QPointF(xi, 0));
    painter.drawLine(QPoint(p.x(), height() - 17),
                     QPoint(p.x(), height() - 20));
    draw_text.setNum(xi);

    painter.drawText(p.x() - 10, height(), QString().setNum(xi));
  }

  for (float yi = ((int)(sceneRect.bottom() / step)) * step;
       yi < sceneRect.top(); yi += step) {

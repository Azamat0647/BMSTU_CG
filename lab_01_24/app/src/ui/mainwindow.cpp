...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    ui->PointsListView->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
    ...
    connect(ui->PointsListView->selectionModel(), &QItemSelectionModel::selectionChanged, this,
            &MainWindow::slot_selection_changed);

    connect(dynamic_cast<ItemDelegate *>(ui->PointsListView->itemDelegate()), &ItemDelegate::item_changed, this,
            &MainWindow::slot_item_changed);
}
...
void MainWindow::slot_selection_changed(const QItemSelection &selected, const QItemSelection &deselected)
{
...
void MainWindow::slot_item_changed(const QModelIndex &index, QString previous_value)
{
    float x = pointsTableModel->data(pointsTableModel->index(index.row(), 0)).toFloat();
    float y = pointsTableModel->data(pointsTableModel->index(index.row(), 1)).toFloat();
...
void MainWindow::on_find_solution_button_clicked()
{
    canvas->find_triangle();
}
...
void MainWindow::on_fileImportAction_triggered()
{
    ...
    while (!f.atEnd())
    {
        ...
        if ((list.length() < 2) or !val->validate(list[0], pos) or !val->validate(list[1], pos))
        {
            clear_table();
